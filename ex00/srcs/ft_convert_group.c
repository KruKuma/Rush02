...
/*   Updated: 2026/07/12 15:03:59 by nfurst           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int ft_print_hundreds(t_dict *tab, char group[4], int *is_first)
{
	...
	if (!ft_print_digit(tab, group[0], is_first))
		return (0);
	if (!ft_print_key(tab, "100", is_first))
		return (0);
    return (1);
}

int	ft_print_tens(t_dict *tab, char group[4], int *is_first)
{
	if (group[1] == '1')
		return (ft_print_two(tab, group[1], group[2], is_first));
	if (group[1] != '0')
	{
	...
	if (group[2] != '0')
    {
        if (!ft_print_digit(tab, group[2], is_first))
            return (0);
    }
    return (1);
}
...
int ft_print_group(t_dict *tab, char *group, int *is_first)
{
    if (!ft_print_hundreds(tab, group, is_first))
        return (0);
    if (!ft_print_tens(tab, group, is_first))
        return (0);
    return (1);
}